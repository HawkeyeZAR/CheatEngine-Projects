...
from tkinter import Tk, Frame, Button, PhotoImage, INSERT, Scrollbar,  Label,\
Text, FALSE, messagebox, Menu, re
import webbrowser
...
class myAOBSigGen(Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.root = parent
        self.root.title(' '*20 +'PyAOB - AOB Signature Generator for Cheat Eng\
        ine Scripts -  by Jack Ackermann')
        self.centreWindow()
        ...
        calc_button = Button(self, text='Generate', background="palegreen1",
                             command=self.getAobText)
        calc_button.grid(column=0, row=6,  sticky='nw', pady=10, padx=15) 
        exit_button = Button(self, text='Exit', background="IndianRed1",
                             command=self.onExit, width=7) 
        ...
        self.aobTextInput.config(font=("Arial",  11),  undo=True, wrap='none',
                                 height=8, width=80)
        ...
        self.script_scrollba = Scrollbar(self, orient='horizontal',
                                         command=self.aobTextInput.xview)
        ...
        self.aobTextOutput.config(font=("Arial",  11), undo=True, wrap='none',
                                  height=8, width=80)
        self.aobTextOutput.grid(row=4, column=0, sticky="new", columnspan=2)
        #self.script_scrollb4 = Scrollbar(self, 
        #command=self.aobTextOutput.yview)
        ...
        self.script_scrollbd = Scrollbar(self, orient='horizontal',
                                         command=self.aobTextOutput.xview)
        ...
        Label(self, text='Paste Your AOB Signatures Here', foreground="purple1",
              background="sky blue",  font=('Arial', 11, 'bold')).grid(column=0, row=0)
        Label(self, text='Your New AOB Signature', foreground="purple1",
              background="sky blue", font=('Arial', 11, 'bold',  'italic')).grid(column=0, row=3)
        ...
        Label(self, text="Shout out to all the great peeps over at CTG",
              foreground="firebrick1",  
                      background="black",  font=('Arial', 11, 'bold')
                      ).grid(column=0,row=7, sticky='ne', pady =2, padx=145)
    ...
    # Get the AOB string signatures entered in the self.aobTextInput textbox,
    # clean the text and pass it to
    ...
    def getAobText(self):
        ...
        # Get the AOB input text from the text box, then split the text so we 
